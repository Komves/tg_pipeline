import os, json, time, math, subprocess, tempfile, shutil, html
...
IN_DIR = Path(r"C:\Users\Марк\Downloads\Telegram Desktop\ChatExport_2026-01-13 (1)\video_files")
PROFILE = Path(r".\out\b_profile.npy")

OUT_JSONL = Path(r".\out\reports\b_rank.jsonl")
OUT_HTML  = Path(r".\out\reports\b_top.html")

FRAMES_PER_VIDEO = 3
TOP_N = 80  # сколько показать в html
IMG_SIZE = 224
...
def cosine(a: np.ndarray, b: np.ndarray) -> float:
    # a,b are already normalized but keep safe
...
def main():
    ...
    print("[rank] in_dir:", IN_DIR)
    print("[rank] videos:", len(vids))
    print("[rank] profile:", PROFILE.resolve())

    device = "cpu"
    model, _, preprocess = open_clip.create_model_and_transforms("ViT-B-32", pretrained="openai", device=device)
    model.eval()

    tmpdir = Path(tempfile.mkdtemp(prefix="brank_"))
    rows = []
    t0 = time.time()
    try:
        for i, vp in enumerate(vids, 1):
            ...
            # score video = max similarity among frames (лучше ловит "есть ли B-сигнал")
            sims = [cosine(v, profile) for v in feats_np]
            score = float(max(sims)) if sims else 0.0

            rows.append({
                "path": str(vp),
                "score": score,
                "duration_s": float(dur) if (dur and dur == dur) else None,
                "frames": len(sims),
            })

            if i % 50 == 0:
                print(f"[progress] {i}/{len(vids)} scored, last={score:.4f}")
    ...
    # HTML отчёт (кликабельно через file:///)
    top = rows[:TOP_N]
    cards = []
    for r in top:
        p = r["path"].replace("\\", "/")
