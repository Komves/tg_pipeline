        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Фактические идентификаторы из переписки нельзя изменять или придумывать.\n"
                    "К ним относятся названия организаций, госорганов, ФИО, номера дел, номера договоров, даты, суммы, контрагенты и названия документов.\n"
                    "Если такой идентификатор есть в письмах, переноси его в request_from, topic и done без переименования, сокращения, расшифровки или улучшения.\n"
                    "Если точного идентификатора нет — не придумывай его.\n"
                    "\n"
...
Главная цель:
Из переписки необходимо сформировать проект акта выполненных консультационных услуг.

Каждая задача должна описывать один законченный или находящийся в работе консультационный вопрос.

1. Не используй тему email как название задачи.

2. Поле topic определи самостоятельно по смыслу ВСЕЙ переписки:
   - входящих писем;
   - исходящих ответов;
   - текста сообщений;
   - названий вложений;
   - содержания вложений (если оно присутствует).

3. Название topic должно позволять через несколько месяцев сразу понять, о каком вопросе шла речь.
...
Хорошо:
- Подготовка ответа на запрос прокуратуры по проверке выплат физическим лицам
- Анализ проекта договора безвозмездного пользования общежитием
- Проверка резюме кандидата на должность юриста
- Подготовка ответа на запрос УМВД о предоставлении документов
- Анализ документов ООО "Чермет-Север" по банковским операциям

4. Поле request_from должно содержать инициатора обращения.
...
5. Поле done должно описывать исключительно выполненную работу.

Не использовать:
- Марголин сделал...
- Марголин подготовил...

Использовать официальный стиль:

- Проведен анализ представленных документов.
- Подготовлен проект договора.
- Подготовлен и направлен ответ с правовым обоснованием.
- Подготовлены замечания к проекту договора.
- Проведен анализ судебной практики.
- Подготовлено юридическое заключение.
- Запрошены дополнительные документы для проведения анализа.

6. Поле done должно содержать 1–3 предложения с конкретным описанием результата.

7. Статус определяй НЕ по тому, есть ли у клиента дальнейшие действия, а по тому, выполнена ли консультационная работа исполнителя.

Статус "завершено" ставь, если из переписки видно, что подготовлен и/или направлен итоговый результат работы:
- ответ;
- письмо;
- проект договора;
- проект документа;
...
Если подготовленный документ содержит рекомендации, перечень дополнительных доказательств, список того, что клиенту нужно собрать дальше, или предложения по усилению позиции — это НЕ означает, что задача "в работе".

Такие рекомендации являются частью уже выполненной консультационной услуги.

8. Статус "в работе" ставь только если из переписки видно, что итоговый результат еще НЕ подготовлен, потому что:
- исполнитель запросил дополнительные документы для начала или завершения анализа;
- ожидаются сведения от клиента;
- продолжается согласование;
- в переписке нет признаков направленного ответа, заключения, доклада или проекта документа.

9. Если исполнитель подготовил доклад, заключение, правовую позицию, проект ответа или иной итоговый документ и направил его адресату — статус всегда "завершено", даже если в документе указано, что нужно дополнительно собрать доказательства или продолжить работу по делу.

10. Если невозможно определить состояние задачи — использовать:

"требует проверки"

11. Не выдумывать факты.

12. Не использовать общие формулировки.

12. Ответ вернуть строго JSON.

TOPIC MUST BE IDENTIFIABLE

13. Поле topic обязано содержать идентификатор дела.

Идентификатор дела = минимум 2 элемента из списка:
- контрагент / организация
- предмет спора или запроса
- тип документа (договор, запрос, проверка, суд, прокуратура)
- номер договора / дела (если есть)
- суть операции (платежи, взыскание, документы, резюме и т.д.)

ЗАПРЕЩЕНО:
- обобщения ("доклад", "анализ", "проверка", "работа")
- абстрактные формулировки без объекта
- темы без контрагента или объекта

ХОРОШИЙ ФОРМАТ:
"ООО РГП — оспаривание платежей по договору поставки"
"Прокуратура — запрос документов по проверке выплат физлицам"
"Отдел кадров — анализ резюме кандидата на должность юриста"
"ООО Чермет-Север — анализ банковских операций и расшифровок счетов"

TOPIC IS NOT A REPORT TYPE

"доклад", "анализ", "сводка", "проверка" — это НЕ тема.

Тема = о чём конкретно кейс.
