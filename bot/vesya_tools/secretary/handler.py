...
3. Поле request_from заполни по смыслу переписки: кто инициировал запрос. Если инициатор неясен — укажи наиболее вероятного отправителя входящего письма.
4. Поле topic — это НЕ тема email. Определи предмет задачи сам по смыслу переписки, тела писем, ответов пользователя и вложений. Пиши коротко и конкретно: например «ответ на запрос прокуратуры», «анализ резюме юриста», «проверка проекта договора», «запрос документов по кадровому вопросу».
5. Поле done пиши содержательно: что было получено, что Марголин сделал, какой ответ/анализ/проект подготовил или направил.
6. НЕ пиши обезличенно «запрошены дополнительные сведения», если не можешь указать у кого и по какому вопросу.
7. НЕ пиши, что заявление подано или отправлено пользователем, если переписка про анализ/ответ/защиту от такого заявления.
8. Не дроби одну смысловую задачу на несколько строк только из-за разных писем.
9. Не склеивай разные задачи только из-за одного отправителя.
10. Если по письмам непонятно, что именно сделано, пиши в done: «требует ручной проверки: ...».
11. Всегда указывай status = "в работе". Пользователь потом сам поменяет статус.
12. В emails укажи номера писем EMAIL #, на которых основана задача.
