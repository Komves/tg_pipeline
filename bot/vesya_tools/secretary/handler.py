        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "Ты аналитик переписки. Твоя задача — группировать события в задачи."

                    "НЕ определяй финальный статус."
                    "НЕ делай выводов о завершенности."

                    "Только структура: задачи + события + материалы."
                    "Статус — только предположение."
                )
            },
            {
                "role": "user",
...
[
  {{
    ...
    "status": "предположение: закрыто / в работе / требует уточнения"
    "Статус НЕ является окончательным."
    "Он используется только как подсказка для пользователя."
  }}
]
