...
def _conversation_reply(
    ...
    system = (
        ...
        "Только в обычном живом разговоре допустимы persona-taglines, колкости и фирменные самокомментарии Веси.\n"
        "В serious object-analysis и discussion режимах такие вставки запрещены.\n"
        ...
        "Если пользователь коротко отвечает на бытовую реплику, напоминание или шутит в ответ — отвечай одной короткой живой фразой.\n"
        "Не разворачивай сценку, мини-рассказ, анализ или цепочку метафор.\n"
        "Допустима только одна шутка или один образ.\n"
        "Не объясняй юмор и не комментируй стиль реплики.\n"
        "Для коротких бытовых ответов: 1 короткая фраза, максимум 8–12 слов.\n"
    )
            ...
            input=[
                {
                    ...
                    "content": (
                        ...
                        "- news: пользователь явно просит общий новостной дайджест/сводку без узкой темы.\n"
                        ...
                        "- актуальный состав групп, расписание, жеребьёвка, турнирная сетка, плей-офф, участники спортивного турнира.\n\n"
                        "Если пользователь просит прогноз по спортивному турниру, группам, плей-офф или матчам — сначала выбирай web_search для проверки текущих фактов. Прогноз без фактической базы запрещён.\n\n"
                        ...
                        "news выбирай только если пользователь явно просит общий дайджест, общую сводку или общие новости.\n"
                        "Если пользователь спрашивает 'что нового' в конкретной теме, отрасли, компании, технологии, городе, стране, персоне или проекте — выбирай web_search, а не news.\n"
                        "Примеры web_search:\n"
                        "- 'что нового в мире ИИ'\n"
                        "- 'что нового у OpenAI'\n"
                        "- 'что нового по рынку металлолома'\n"
                        "- 'что нового в Нижневартовске'\n"
                        "- 'последние новости по REFRESHTECH'\n\n"
                        ...
                        "Для пользовательских просьб 'дай/найди/покажи/скинь/пришли/подбери что-нибудь' НЕ считай это обычным chat.\n"
                        "Если объект запроса — музыкальная группа, исполнитель, песня, трек, клип, live или концерт — intent=youtube_search.\n"
                        "Примеры youtube_search:\n"
                        "- 'дай что-нибудь из Offspring на твой вкус' => intent=youtube_search, query='Offspring official video'\n"
                        "- 'найди клип Metallica' => intent=youtube_search, query='Metallica official video'\n"
                        "- 'покажи live Queen' => intent=youtube_search, query='Queen live'\n"
                        "- 'подбери песню под вечер' => intent=youtube_search, query='песня под вечер'\n"
                        "Если пользователь просит фото/фотки/картинки/изображения человека, места или объекта — intent=web_search.\n"
                        "Примеры web_search:\n"
                        "- 'дай фото Саши Грей' => intent=web_search, query='фото Саши Грей'\n"
                        "- 'покажи фотки Нижневартовска зимой' => intent=web_search, query='фото Нижневартовск зимой'\n"
                        "- 'найди картинки лодки ПВХ' => intent=web_search, query='картинки лодка ПВХ'\n"
                        "content используй только для внутренних подборок Веси: мемы, видосы, огня, get12/get24, beauty-контент.\n"
                        "chat используй только для разговора, мнения, объяснения, шутки или обсуждения без просьбы найти/дать/показать внешний объект.\n\n"
...
def describe_or_compare_photo(text: str, img_bytes: bytes) -> Optional[DialogDecision]:
    ...
    try:
        ...
        asks_about_image = bool(re.search(
            ...
            r"что думаешь|как думаешь|"
            r"прокомментируй|разбери|"
            r"поясни|объясни|"
            r"это правда|ну и|и что|когда уже|"
            r"как тебе|"
            r"зацени|"
            r"оцени|"
            r"твое мнение|твоё мнение|"
            r"что скажешь"
            r")\b",
            tl,
        ))
        ...
        if asks_about_image:
            prompt = (
                "Пользователь прислал изображение и просит именно реакцию/мнение Веси.\n"
                f"Текст запроса пользователя: {user_task}\n\n"

                "Главная задача — дать личную реакцию Веси, а НЕ описание картинки.\n"
                "НЕ пересказывай изображение.\n"
                "НЕ начинай с фраз:\n"
                "- 'на изображении'\n"
                "- 'это постер'\n"
                "- 'это мем'\n"
                ...
                "Сначала должна идти реакция/оценка/ирония Веси.\n"
                "Описание допускается только если без него вообще не понять мысль.\n"

                "Если это мем, постер, шутка, политический материал или скрин — "
                "реагируй именно на смысл, подтекст и настроение.\n"

                "Формат:\n"
                "- 1–3 короткие фразы;\n"
                "- без школьного разбора;\n"
                "- без пересказа;\n"
                "- без анализа композиции картинки;\n"
                "- без третьего лица;\n"
                "- без вопросов пользователю.\n"

                "Стиль Веси:\n"
                "сухо, умно, колко, по сути."
            )
        elif any(x in tl for x in ("шутк", "мем", "смешн", "юмор")):
            ...
            prompt = (
                "Проанализируй именно изображение.\n"
                f"Текст запроса пользователя: {user_task}\n\n"
                "Учитывай текст запроса пользователя как задачу к фото.\n"
                "Если пользователь просит оценить вес, возраст, телосложение, состояние, предмет, сцену или объект — "
                "делай оценку по видимым признакам на изображении, а не по общим справочным диапазонам.\n"
                "Сначала используй визуальные признаки: поза, пропорции, одежда, корпус, руки, живот, масштаб, перспектива, видимость объекта.\n"
                "Если часть объекта плохо видна — прямо скажи, что оценка грубая и почему.\n"
                "Не отвечай только теорией вроде 'по одному росту не считают', если фото даёт визуальные данные.\n"
                "Если это культурный объект, артефакт или предмет — попробуй определить его как можно точнее.\n"
                "Ответ: коротко, по делу, по-русски, без вопросов пользователю."
            )
            ...
            input=[
                {
                    ...
                    "content": (
                        ...
                        + _runtime_context_block()
                        + "\n\n"
                        "Ты — Веся. Пользователь показал внешний объект: текст, пост, объявление, ссылку, описание или пересланное сообщение.\n\n"
                        "Главная задача — строго исполнить запрос пользователя к этому объекту.\n"
                        "Запрос пользователя может быть не исходной фразой, а instruction-контрактом от semantic-router.\n"
                        "Если в запросе указано, какой тип ответа нужен и чего нельзя делать — выполняй это буквально.\n"
                        "Это serious object-analysis режим.\n"
                        "Запрещены канонные self-taglines и persona-вставки вроде:\n"
                        "- 'я людей читаю профессионально'\n"
                        "- 'я такое вижу сразу'\n"
                        "- 'не вчера родилась'\n"
                        "- 'я в людях разбираюсь'\n"
                        "- любые самодовольные подписи в конце.\n"
                        "Не ломай серьёзный аналитический тон persona-репликами.\n"
                        "Если instruction требует мнение или оценку — не уходи в юридический пересказ, фактчек или нейтральную сводку.\n"
                        "Если запрос пользователя похож на 'как тебе', 'что думаешь', 'что скажешь', 'твое мнение', 'твоё мнение', 'оцени', 'прокомментируй' — "
                        "это просьба дать личную оценку, а НЕ пересказать объект.\n\n"
                        "В режиме личной оценки:\n"
                        "- НЕ начинай с 'это новость', 'это текст', 'суть такая', 'речь идет', 'там говорится'.\n"
                        "- НЕ пересказывай условия пунктами.\n"
                        "- Сначала дай позицию Веси, а не пересказ. Для новостей про законы/льготы/долги смотри в корень: кто получает выгоду, кто несет нагрузку, кому это политически удобно.\n"
                        "- Если новость про списание долгов участникам войны/СВО за счет кредиторов, нормальная оценка: это не просто льгота, а способ стимулировать набор людей, перекладывая часть цены войны на банковскую систему. Формулируй мягче, без грубости.\n"
                        "- Можно упомянуть максимум одну ключевую деталь объекта как опору для мнения, но не пересказывай условия списком.\n"
                        "- Запрещено писать второй абзац в формате пересказа: 'условия такие', 'контракт должен', 'долг должен', 'лимит'. Это не мнение, а реферат.\n"
                        "- Не делай фактчек без прямой просьбы 'это правда?', 'фейк?', 'проверь'.\n"
                        "- Не говори 'фейк' или 'правда', если достоверность явно не спрашивали.\n"
                        "- Не называй дату будущей, не сверившись с RUNTIME CONTEXT.\n"
                        "- Если RUNTIME CONTEXT показывает дату позже даты из объекта, запрещено писать, что дата объекта находится в будущем.\n\n"
                        "Если пользователь просит 'что это', 'о чем это', 'перескажи', 'объясни' — тогда можно кратко описать смысл объекта.\n"
                        ...
                        "Формат по умолчанию: 2–4 короткие фразы. Без вопросов пользователю.\n"
                        "Если instruction требует мнение — сначала дай вывод/позицию, потом максимум одну опорную деталь.\n"
                        "Веся имеет право делать аналитические и причинно-следственные выводы о мотивации, выгоде, механике и скрытом смысле решений, если это логически следует из объекта.\n"
                        "Если объект описывает закон, льготу, экономическую меру, ограничение или государственное решение — Веся должна анализировать реальный механизм действия, а не только формальный текст.\n"
                        "Если пользователь спрашивает мнение, запрещено уходить в нейтральный пересказ, media-literacy советы или бесконечную осторожность вроде 'нужно проверять источник', если пользователь не спрашивал про достоверность.\n"
                        "Если instruction требует пересказ — кратко перескажи.\n"
                        "Если instruction требует проверку достоверности — оцени осторожно и не делай категоричный вывод без оснований.\n"
                        "Если instruction запрещает пересказ, фактчек или мета-комментарий про источник — не делай этого."
                    ),
                },
                {
                    "role": "user",
...
def semantic_context_route(
    ...
    *,
    ...
    if re.search(
        r"^(напомни|напоминание|поставь\s+напоминание|добавь\s+в\s+календарь|запланируй)\b",
        calendar_text,
        flags=re.I,
    ):
        return {
    ...
    try:
            ...
            input=[
                {
                    ...
                    "content": (
                        ...
                        "4. calendar — пользователь просит создать напоминание, поставить событие, добавить в календарь или запланировать действие.\n"
                        "5. translate_factcheck — пользователь просит перевести текст и одновременно проверить/подтвердить факты по внешним независимым источникам.\n\n"
                        "Правила:\n"
                        ...
                        "И current_object не содержит нового самостоятельного объекта для анализа.\n"
                        "- Если current_message содержит просьбу напомнить, поставить напоминание, добавить в календарь или запланировать действие — выбирай calendar.\n"
                        "- calendar выбирай для фраз вроде: напомни завтра, напомни через час, поставь напоминание, добавь в календарь, запланируй.\n"
                        "- Если current_object содержит новый текст, ссылку, пересланное сообщение, фото, документ или иной объект, "
                        "сначала сравни current_object с previous_topic.\n"
                        "- Если current_object отличается от previous_topic по предмету обсуждения, выбирай object_analysis.\n"
                        "- Если пользователь говорит 'это', 'тут', 'здесь', 'сообщение', 'ссылка', 'пересылка', 'на фото' "
                        "и одновременно есть current_object — обычно это указание на current_object, а не на previous_topic.\n"
                        "- topic_followup при наличии current_object допустим только если current_object явно является продолжением той же самой темы "
                        "или пользователь явно связывает вопрос с previous_topic.\n"
                        ...
                        "- Если пользователь просит напомнить, поставить напоминание, добавить в календарь, запланировать действие или настроить свой часовой пояс — выбирай calendar.\n"
                        "- Не зашивай частные сценарии вроде Avito, вес, рост, музыка.\n\n"
                        "instruction — это ОБЯЗАТЕЛЬНЫЙ контракт для следующего LLM-исполнителя.\n"
                        "В instruction ясно укажи:\n"
                        "1) что именно нужно сделать с объектом;\n"
                        "2) какой тип ответа нужен: мнение / объяснение / краткий пересказ / проверка достоверности / разбор;\n"
                        "3) чего нельзя делать.\n\n"
                        "Если пользователь просит мнение, реакцию или 'как тебе' — instruction должен требовать личную оценку Веси по смыслу объекта, "
                        "а не пересказ и не фактчек.\n"
                        "Если пользователь просит объяснить или пересказать — instruction должен требовать краткое объяснение/пересказ.\n"
                        "Если пользователь явно спрашивает 'фейк?', 'правда?', 'проверь достоверность' — instruction должен требовать осторожную проверку достоверности.\n"
                        "Не зашивай частные сценарии. Решай по смыслу текущего запроса и объекта.\n\n"
                        "Верни только JSON:\n"
                        "{\"route\":\"chat|object_analysis|topic_followup|calendar|translate_factcheck\",\"instruction\":\"обязательный контракт для исполнителя: что сделать, какой тип ответа, чего не делать\"}"
                    ),
                },
                {
                    "role": "user",
        ...
        if route not in {"chat", "object_analysis", "topic_followup", "calendar", "translate_factcheck"}:
            route = "chat"
            ...
            input=[
                {
                    ...
                    "content": (
                        ...
                        "instruction — это ОБЯЗАТЕЛЬНЫЙ контракт для будущего анализа объекта.\n"
                        "В instruction ясно укажи:\n"
                        "1) что именно нужно сделать с будущим объектом;\n"
                        "2) какой тип ответа нужен: мнение / объяснение / краткий пересказ / проверка достоверности / разбор;\n"
                        "3) чего нельзя делать.\n\n"
                        "Если пользователь просит мнение, реакцию или 'как тебе' — instruction должен требовать личную оценку Веси по смыслу объекта, "
                        "а не пересказ и не фактчек.\n"
                        "Если пользователь просит объяснить или пересказать — instruction должен требовать краткое объяснение/пересказ.\n"
                        "Если пользователь явно спрашивает 'фейк?', 'правда?', 'проверь достоверность' — instruction должен требовать осторожную проверку достоверности.\n"
                        "Не зашивай частные сценарии. Решай по смыслу текущего запроса.\n\n"
                        "Верни только JSON:\n"
                        "{\"wait_for_object\":true|false,\"instruction\":\"обязательный контракт для исполнителя: что сделать, какой тип ответа, чего не делать\"}"
                    ),
                },
                {
                    "role": "user",
            ...
            input=[
                {
                    ...
                    "content": (
                        ...
                        "Определи, просит ли пользователь прислать один ролик из подготовленного beauty/aesthetic-пула.\n"
                        "Это НЕ просьба оценить присланное видео и НЕ обычный разговор.\n"
                        "Команда может быть сформулирована свободно, не по шаблону.\n\n"
                        "Смысл true:\n"
                        "- пользователь хочет, чтобы Веся прислала красивый/эстетичный/вайбовый ролик;\n"
                        "- пользователь просит красоты, красивое, эстетики, вайба, приятный ролик;\n"
                        "- пользователь просит 'сделай красиво' как действие отправки контента.\n\n"
                        "Примеры true:\n"
                        "- Веся сделай красиво\n"
                        "- дай красоты\n"
                        "- покажи красивое\n"
                        "- красоту хочу\n"
                        "- пришли что-нибудь красивое\n"
                        "- дай эстетики\n"
                        "- хочу вайба\n"
                        "- порадуй красивым роликом\n"
                        "- что там у нас красивого\n\n"
                        "Примеры false:\n"
                        "- красиво получилось?\n"
                        "- оцени, красиво ли это видео\n"
                        "- как тебе эта девушка\n"
                        "- что думаешь про ролик\n"
                        "- красиво сказано\n\n"
