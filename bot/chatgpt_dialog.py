    ...
    try:
        ...
        prompt = (
           "Ты фильтр контента для телеграм-бота. Реши, можно ли отправлять картинку как мем.\n"
            ...
            "РАЗРЕШАЙ (ok=true), если это похоже на мем ИЛИ даже просто нормально выглядит как мем.\n"
            "Не требуй сильного панча: допускай умеренно смешные/тупые/простые мемы.\n"
            "Правило: если сомневаешься — ok=true, КРОМЕ явного запрета (NSFW/личное/реклама/витрина).\n"
...
def meme_rank_batch(
    ...
    *,
    ...
    prompt_lines = [
        "Ты — СТРОГИЙ ранжировщик мемов для телеграм-бота.",
        "Цель: выбрать ТОЛЬКО реально смешные/мемные картинки. Мусор и спорное — отклоняй.",
        "",
        "ЖЁСТКИЙ BAN (ok=false), если есть хоть один признак:",
        "- политика/геополитика/война/митинги/пропаганда/агитки",
        "- гос-символика, флаги, гербы, эмблемы ведомств, официальные плакаты",
        "- официальные портреты/пресс-фото политиков/чиновников/военных",
        "- новости/скрин новостей/заголовки/тв-кадры/официальные заявления",
        "- NSFW: сексуальный контент/обнажёнка/порно/фетиш",
        "- реклама/промо/магазин/розыгрыш/казино/крипта/подписки/промокоды/донаты",
        "- “просто картинка”: пейзаж/еда/товар/инфографика/объявление/скрин витрины",
        "- личное фото/селфи/частное фото без мемного смысла",
        "- AI-арт/нейросеточная картинка БЕЗ явной шутки/панча (если это просто “красиво” — BAN)",
        "- скрин переписки/поста без чёткой шутки (если не смешно само по себе — BAN)",
        "",
        "РАЗРЕШАЙ (ok=true) ТОЛЬКО если:",
        "- есть явная шутка/ирония/сарказм/панч (обычно текст на картинке или узнаваемый мем-формат)",
        "- и это безопасно (нет политики/NSFW/рекламы)",
        "",
        "ПРАВИЛО СОМНЕНИЯ: если сомневаешься — ставь ok=false.",
        "Если нормальных мемов нет — это нормально. Верни picked_item_ids: [].",
        "",
        "Верни СТРОГО валидный JSON (без пояснений):",
        "{",
        '  "items": [',
        '    {"idx":0,"item_id":"...","ok":true|false,"score":0-100,"reason":"2-8 слов","tags":["..."]},',
        "    ...",
        "  ],",
        '  "picked_item_ids": ["..."]',
        "}",
        ...
        "- score: 0..100 (чем мемнее/смешнее, тем выше).",
        "- reason: 2–8 слов (например: 'реклама', 'скрин без шутки', 'мем с текстом', 'реакция').",
        "- tags: 0–5 тегов (например: 'text', 'reaction', 'screenshot', 'ad-like', 'personal', 'politics').",
        "",
        "Метаданные caption/src — слабый сигнал, картинка важнее.",
