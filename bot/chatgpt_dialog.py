...
def describe_or_compare_photo(text: str, img_bytes: bytes) -> Optional[DialogDecision]:
    ...
    try:
        ...
        if any(x in tl for x in ("шутк", "мем", "смешн", "юмор", "оцени", "оценить")):
            prompt = (
                "Пользователь просит оценить мем, шутку или юмор на изображении.\n"
                f"Текст запроса пользователя: {user_task}\n\n"
                "Разбери именно шутку, а не просто объект на картинке.\n"
                "Объясни, в чём смысл шутки.\n"
                "Оцени, насколько она смешная/удачная.\n"
                "Если это игра слов, культурный контекст или скрытый смысл — объясни коротко.\n"
                "Отвечай как Веся: умно, сухо, с лёгкой иронией, без энциклопедического тона.\n"
                "Без вопросов пользователю. По-русски."
            )
        else:
            prompt = (
