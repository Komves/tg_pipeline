...
def read_rank(jsonl_path: Path):
    items = []
    # jsonl is utf-8 (written by python). Use utf-8 and ignore bad bytes just in case.
    for line in jsonl_path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if score is None:
            continue
        # path may be under different keys depending on normalization
        p = o.get("path") or o.get("src") or o.get("orig_path") or o.get("tmp_path")
        if not p:
            continue
        items.append({"score": float(score), "path": str(p)})
...
function render(){{
 ...
 meta.innerHTML="<b>"+(i+1)+"/"+data.length+"</b> score="+v.score.toFixed(4)+" | current="+cur+" | OK="+count("OK")+" NO="+count("NO")+" SKIP="+count("SKIP");
...
def main(topn: int = 30, out_name: str = "review_A.html"):
    cfg = load_cfg()
    reports = Path(cfg["reports_dir"])
    rank = reports / "b_rank_daily.jsonl"
    if not rank.exists():
        raise SystemExit(f"missing rank: {rank}")

    items = read_rank(rank)
    if not items:
        raise SystemExit("no OK items parsed from jsonl (python)")
    ...
    if not data:
        raise SystemExit("top items exist, but none of paths exist on disk")

    js = json.dumps(data, ensure_ascii=False)
    b64 = base64.b64encode(js.encode("utf-8")).decode("ascii")

    html = build_html(b64)
    out_html = ROOT / out_name
    out_html.write_text(html, encoding="utf-8")

    print("OK. HTML:", out_html)
    print("Videos:", len(data), "->", vid_dir)

if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser()
    ap.add_argument("--topn", type=int, default=30)
    ap.add_argument("--out", type=str, default="review_A.html")
    args = ap.parse_args()
    main(args.topn, args.out)
